...
class PlatformSerializableSavedStateMachineTest {
    ...
    fun testSaveState_givenUnsupportedState_thenThrowsException() {
        ...
        val handle = SavedStateHandle()
        PlatformSerializableStateMachine(handle, "key", expect)

        assertFailure { handle.savedStateProvider().saveState() }
            .isInstanceOf(IllegalArgumentException::class)
            .hasMessage("Illegal value type io.github.ryunen344.mutton.savedstate.testing.PlatformSerializableState.Error for key \"key\"")
    }
}
